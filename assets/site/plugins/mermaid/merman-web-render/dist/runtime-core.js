import { createMermanRuntimeState, currentMermanRuntimeState, } from "./runtime-state.js";
import { MERMAN_TEXT_MEASUREMENT_PROTOCOL_VERSION, } from "./generated/text-measurement-abi.js";
import { BINDING_OPTION_GROUP_SPECS, BINDING_PAYLOAD_SCHEMAS, BINDING_TRANSPORT_EXPOSURE_SPECS, CAPABILITY_SPECS, CONSTRUCTOR_SERVICE_SPECS, METADATA_SPECS, RUNTIME_CATALOG_FIELD_IDENTIFIER_PATTERN, RUNTIME_CATALOG_IDENTIFIER_PATTERN, RUNTIME_CATALOG_MAX_SAFE_INTEGER, RUNTIME_CATALOG_SCHEMA_VERSION, TEXT_MEASUREMENT_PROVIDER_SPECS, } from "./generated/binding-contract.js";
import { WEB_BINDING_OPERATIONS, } from "./generated/capability-surface.js";
import { BINDING_OPTIONS_SCHEMA_VERSION, tightenResourceOptions, } from "./generated/resource-contract.js";
import { isDiagramType, isThemeName, } from "./public-catalog.js";
...
const RUNTIME_CATALOG_FIELD_IDENTIFIER = new RegExp(RUNTIME_CATALOG_FIELD_IDENTIFIER_PATTERN);
const BINDING_OPTION_GROUP_SPEC_BY_ID = new Map(BINDING_OPTION_GROUP_SPECS.map((spec) => [spec.id, spec]));
const CONSTRUCTOR_SERVICE_SPEC_BY_ID = new Map(CONSTRUCTOR_SERVICE_SPECS.map((spec) => [spec.id, spec]));
const WEB_TRANSPORT_EXPOSURE = BINDING_TRANSPORT_EXPOSURE_SPECS.find((spec) => spec.id === "web");
if (WEB_TRANSPORT_EXPOSURE === undefined) {
    throw new Error("Generated binding contract is missing the Web transport exposure.");
}
const WEB_BINDING_OPERATION_SPEC_BY_ID = new Map(WEB_BINDING_OPERATIONS.map((spec) => [spec.id, spec]));
const WEB_PAYLOAD_SCHEMA_VERSION_BY_ID = new Map(WEB_TRANSPORT_EXPOSURE.payload_schema_ids.map((id) => {
    ...
    return [id, spec.version];
}));
const WEB_CONSTRUCTOR_SERVICE_CANDIDATE_IDS = new Set(WEB_TRANSPORT_EXPOSURE.constructor_service_candidate_ids);
const TEXT_MEASUREMENT_PROVIDER_SPEC_BY_ID = new Map(TEXT_MEASUREMENT_PROVIDER_SPECS.map((spec) => [spec.id, spec]));
export function currentRuntimeState() {
...
async function doInit(state, init) {
...
async function defaultLoader() {
    throw new Error("The shared @mermanjs/web implementation has no WASM artifact. Import one browser package entry such as @mermanjs/web, @mermanjs/web-render, @mermanjs/web-analysis, @mermanjs/web-editor, or @mermanjs/web-ascii.");
}
export function getMerman() {
    const state = currentRuntimeState();
    if (!state.wasmModule) {
...
export function isMermanInitialized() {
    return currentRuntimeState().wasmModule !== null;
}
export const UNAVAILABLE_DIAGRAM_DETECTION = Object.freeze({
    status: "unavailable",
    validity: "unknown",
    diagramType: null,
    syntaxId: null,
...
export function runtimeCatalog() {
    const state = currentRuntimeState();
    state.runtimeCatalogCache ?? (state.runtimeCatalogCache = normalizeRuntimeCatalog(getMerman().runtimeCatalog()));
    return structuredCloneValue(state.runtimeCatalogCache);
}
export function presentationCatalog() {
    const state = currentRuntimeState();
    state.presentationCatalogCache ?? (state.presentationCatalogCache = normalizePresentationCatalog(getMerman().presentationCatalog()));
    return structuredCloneValue(state.presentationCatalogCache);
}
export function supportedDiagrams() {
    const state = currentRuntimeState();
    state.supportedDiagramsCache ?? (state.supportedDiagramsCache = getMerman().supportedDiagrams().map(assertDiagramType));
    return [...state.supportedDiagramsCache];
}
export function diagramFamilyCapabilities() {
    return cachedDiagramFamilyCapabilities().map((capability) => ({ ...capability }));
}
function cachedDiagramFamilyCapabilities() {
    const state = currentRuntimeState();
    state.diagramFamilyCapabilitiesCache ?? (state.diagramFamilyCapabilitiesCache = getMerman()
...
export function supportedThemes() {
    const state = currentRuntimeState();
    state.supportedThemesCache ?? (state.supportedThemesCache = getMerman().supportedThemes().map(assertThemeName));
    return [...state.supportedThemesCache];
}
export function transportApiVersion() {
    return assertSafeIntegerField(getMerman().transportApiVersion(), "Web transport API version", 1);
}
...
export function withResourceOptions(options, resources) {
    const result = { ...options };
    const wrappers = ["analysis", "merman"].filter((key) => Object.prototype.hasOwnProperty.call(options, key));
    if (wrappers.length > 1) {
        throw new TypeError("Merman options must not contain both analysis and merman wrappers.");
    }
    if (wrappers.length !== 0 &&
        Object.prototype.hasOwnProperty.call(options, "resources")) {
        throw new TypeError("Merman options must not mix top-level resources with an analysis or merman wrapper.");
    }
    ...
    if (wrapperKey !== undefined) {
        ...
        result[wrapperKey] = {
            ...wrapper,
            resources: tightenResourceOptions(resources, wrapper.resources),
        };
    }
    else {
...
function assertDiagramType(diagram) {
...
function normalizeDiagramFamilyCapability(capability) {
    ...
    const metadataId = capability.metadata_id === undefined || capability.metadata_id === null
        ? null
        : assertDiagramType(String(capability.metadata_id));
    return {
        diagram_type: capability.diagram_type,
        logical_family_kind: assertStringField(capability.logical_family_kind, "diagram logical family kind"),
        metadata_id: metadataId,
        render_model_kind: assertNullableStringField(capability.render_model_kind, "diagram render model kind"),
        has_detector: assertBooleanField(capability.has_detector, "diagram detector capability"),
        has_semantic_parser: assertBooleanField(capability.has_semantic_parser, "diagram semantic parser capability"),
        has_editor_parser: assertBooleanField(capability.has_editor_parser, "diagram editor parser capability"),
        has_combined_parser: assertBooleanField(capability.has_combined_parser, "diagram combined parser capability"),
        has_render_parser: assertBooleanField(capability.has_render_parser, "diagram render parser capability"),
        has_header: assertBooleanField(capability.has_header, "diagram header capability"),
        config_namespace: assertNullableStringField(capability.config_namespace, "diagram config namespace"),
    };
}
...
function assertNullableStringField(value, label) {
...
function assertThemeName(theme) {
...
function normalizeRuntimeCatalog(value) {
    ...
    const catalogTransportApiVersion = assertSafeIntegerField(value.transport_api_version, "runtime transport API version", 1);
    if (catalogTransportApiVersion !== transportApiVersion()) {
        throw new Error("Merman WASM runtime catalog transport API does not match the loaded module.");
    }
    if (typeof value.package_version !== "string" ||
    ...
    assertRequiredRecordKeys(value.registry, ["diagram_family_count"], "Merman WASM runtime registry catalog");
    const diagramFamilyCount = assertSafeIntegerField(value.registry.diagram_family_count, "runtime registry diagram family count", 0);
    ...
    const optionsSchemaVersions = normalizeSortedPositiveIntegers(value.options_schema_versions, "runtime options schema versions");
    if (!optionsSchemaVersions.includes(BINDING_OPTIONS_SCHEMA_VERSION)) {
        throw new Error(`Merman WASM runtime catalog does not advertise options schema v${BINDING_OPTIONS_SCHEMA_VERSION}.`);
    }
    return {
        ...
        capabilities,
        output_contracts: normalizeRuntimeOutputContracts(value.output_contracts, capabilities.output_ids),
        registry: {
            ...structuredCloneValue(value.registry),
            diagram_family_count: diagramFamilyCount,
        },
        resources: normalizeRuntimeResourceContract(value.resources, new Set(capabilities.operation_ids)),
    };
}
function runtimeUsesSvgPipeline(capabilities) {
    return capabilities.capability_ids.includes("svg") || capabilities.text_measurement !== null;
}
function normalizeRuntimeMetadataIds(value, capabilities) {
    ...
    for (const id of ids) {
        ...
        if (spec?.required_capability_id !== null &&
            spec?.required_capability_id !== undefined &&
            !capabilityIds.has(spec.required_capability_id)) {
            throw new Error(`Merman WASM runtime metadata ${id} requires capability ${spec.required_capability_id}.`);
        }
    }
    return ids;
}
function normalizeRuntimeOptionGroupIds(value, capabilities, usesSvgPipeline) {
    const ids = normalizeSortedOptionGroupIds(value);
    const capabilityIds = new Set(capabilities.capability_ids);
    const expectedKnownIds = BINDING_OPTION_GROUP_SPECS
        .filter((spec) => spec.always_available ||
    ...
    if (!sameStringArrays(actualKnownIds, expectedKnownIds)) {
        throw new Error("Merman WASM runtime option group IDs do not match the artifact capability closure.");
    }
    return ids;
}
function normalizeRuntimeConstructorServices(catalog, capabilities) {
    const hasIds = Object.prototype.hasOwnProperty.call(catalog, "constructor_service_ids");
    const hasContracts = Object.prototype.hasOwnProperty.call(catalog, "constructor_service_contracts");
    if (hasIds !== hasContracts) {
        throw new Error("Merman WASM runtime constructor service IDs and contracts must appear together.");
    }
    if (!hasIds) {
        return { ids: [], contracts: [] };
    }
    const ids = normalizeSortedIdentifierIds(catalog.constructor_service_ids, "runtime constructor service IDs");
    const usesSvgPipeline = runtimeUsesSvgPipeline(capabilities);
    for (const id of ids) {
        ...
        if (!WEB_CONSTRUCTOR_SERVICE_CANDIDATE_IDS.has(id)) {
            throw new Error(`Merman WASM advertises constructor service ${id}, which this Web facade cannot provide.`);
        }
        if (knownSpec.requires_svg_pipeline && !usesSvgPipeline) {
            throw new Error(`Merman WASM advertises constructor service ${id} without an SVG pipeline.`);
        }
    }
    const contracts = normalizeRuntimeConstructorServiceContracts(catalog.constructor_service_contracts);
    const contractIds = contracts.map((contract) => contract.id);
    if (!sameStringArrays(ids, contractIds)) {
        throw new Error("Merman WASM runtime constructor service IDs and contracts must be in one-to-one correspondence.");
    }
    validateRuntimeConstructorServiceProviders(contracts, capabilities);
    return { ids, contracts };
}
function normalizeRuntimeConstructorServiceContracts(value) {
    ...
    const contracts = value.map((contract) => {
        ...
        assertRequiredRecordKeys(contract, ["id", "provided_text_measurement_provider_ids", "resource_limits"], "Merman WASM runtime constructor service contract");
        const id = assertRuntimeIdentifier(contract.id, "runtime constructor service contract ID");
        const providerIds = normalizeSortedIdentifierIds(contract.provided_text_measurement_provider_ids, `runtime constructor service ${id} text measurement provider IDs`);
        return {
    ...
    for (let index = 1; index < ids.length; index += 1) {
        if (ids[index - 1] >= ids[index]) {
            throw new Error("Merman WASM runtime constructor service contracts must be sorted and unique by ID.");
        }
    }
    return contracts;
}
function normalizeRuntimeConstructorResourceLimits(value, serviceId) {
    if (!Array.isArray(value)) {
        throw new Error(`Merman WASM returned invalid runtime constructor service ${serviceId} resource limits.`);
    }
    const limits = value.map((limit) => {
        if (!isRecord(limit)) {
            throw new Error(`Merman WASM returned an invalid runtime constructor service ${serviceId} resource limit.`);
        }
        assertRequiredRecordKeys(limit, ["id", "phase", "unit", "description", "value"], `Merman WASM runtime constructor service ${serviceId} resource limit`);
        const id = assertRuntimeFieldIdentifier(limit.id, `runtime constructor service ${serviceId} resource limit ID`);
        if (typeof limit.phase !== "string" ||
        ...
        return {
            ...
            value: assertSafeIntegerField(limit.value, `runtime constructor service ${serviceId} resource limit value`, 0),
        };
    });
    const ids = limits.map((limit) => limit.id);
    for (let index = 1; index < ids.length; index += 1) {
        if (ids[index - 1] >= ids[index]) {
            throw new Error(`Merman WASM runtime constructor service ${serviceId} resource limits must be sorted and unique by ID.`);
        }
    }
    return limits;
}
function validateRuntimeConstructorServiceProviders(contracts, capabilities) {
    const runtimeProviderIds = new Set(capabilities.text_measurement?.provider_ids ?? []);
    const contractById = new Map(contracts.map((contract) => [contract.id, contract]));
    const providerOwnerById = new Map();
    for (const contract of contracts) {
        const serviceSpec = CONSTRUCTOR_SERVICE_SPEC_BY_ID.get(contract.id);
        if (serviceSpec !== undefined) {
            const actualKnownProviderIds = contract.provided_text_measurement_provider_ids.filter((id) => TEXT_MEASUREMENT_PROVIDER_SPEC_BY_ID.has(id));
            if (!sameStringArrays(actualKnownProviderIds, [...serviceSpec.provided_text_measurement_provider_ids])) {
                throw new Error(`Merman WASM runtime constructor service ${contract.id} does not match its known provider contract.`);
            }
        }
        for (const providerId of contract.provided_text_measurement_provider_ids) {
            if (!runtimeProviderIds.has(providerId)) {
                throw new Error(`Merman WASM runtime constructor service ${contract.id} provides an unavailable text measurement provider.`);
            }
            const previousOwner = providerOwnerById.get(providerId);
            if (previousOwner !== undefined) {
                throw new Error(`Merman WASM runtime text measurement provider ${providerId} has multiple constructor service owners.`);
            }
            ...
            if (providerSpec !== undefined &&
                ...
                throw new Error(`Merman WASM runtime text measurement provider ${providerId} has the wrong constructor service owner.`);
            }
        }
    }
    for (const providerId of runtimeProviderIds) {
        ...
        if (ownerContract === undefined ||
            ...
            throw new Error(`Merman WASM runtime text measurement provider ${providerId} is missing its constructor service owner.`);
        }
    }
}
function normalizePresentationCatalog(value) {
    if (!isRecord(value) || value.schema_version !== 1) {
        throw new Error("Merman WASM returned an unsupported presentation catalog schema.");
    }
    assertRequiredRecordKeys(value, ["profiles", "schema_version", "theme_presets"], "Merman WASM presentation catalog");
...
function normalizePresentationThemePresets(value) {
    const seen = new Set();
    return value.map((entry) => {
        if (!isRecord(entry)) {
            throw new Error("Merman WASM returned an invalid presentation theme preset.");
        }
        assertRequiredRecordKeys(entry, ["appearance", "fully_available", "id", "missing_capability_ids"], "Merman WASM presentation theme preset");
        const id = assertUniquePresentationId(entry.id, seen, "theme preset");
        return {
            id,
            appearance: assertRuntimeIdentifier(entry.appearance, `presentation theme preset ${id} appearance`),
            fully_available: assertBooleanField(entry.fully_available, `presentation theme preset ${id} availability`),
            missing_capability_ids: normalizeSortedIdentifierIds(entry.missing_capability_ids, `presentation theme preset ${id} missing capability IDs`),
        };
    });
}
function normalizePresentationProfiles(value) {
    const seen = new Set();
    return value.map((entry) => {
        if (!isRecord(entry)) {
            throw new Error("Merman WASM returned an invalid presentation profile.");
        }
        assertRequiredRecordKeys(entry, ["aspects", "fully_available", "id", "missing_capability_ids"], "Merman WASM presentation profile");
        ...
        return {
            id,
            fully_available: assertBooleanField(entry.fully_available, `presentation profile ${id} availability`),
            missing_capability_ids: normalizeSortedIdentifierIds(entry.missing_capability_ids, `presentation profile ${id} missing capability IDs`),
            aspects: normalizePresentationAspects(id, entry.aspects),
        };
    });
}
function normalizePresentationAspects(profileId, value) {
    const seen = new Set();
    return value.map((entry) => {
        ...
        assertRequiredRecordKeys(entry, [
            "applicability",
            "available",
            "id",
        ...
        assertRequiredRecordKeys(entry.applicability, ["family_id", "kind"], "Merman WASM presentation aspect applicability");
        const id = assertUniquePresentationId(entry.id, seen, `profile ${profileId} aspect`);
        const familyId = entry.applicability.family_id;
        return {
            id,
            applicability: {
                kind: assertRuntimeIdentifier(entry.applicability.kind, `presentation aspect ${id} applicability kind`),
                family_id: familyId === null
                    ? null
                    : assertRuntimeIdentifier(familyId, `presentation aspect ${id} family ID`),
            },
            required_capability_id: entry.required_capability_id === null
                ? null
                : assertRuntimeIdentifier(entry.required_capability_id, `presentation aspect ${id} required capability ID`),
            available: assertBooleanField(entry.available, `presentation aspect ${id} availability`),
            missing_capability_ids: normalizeSortedIdentifierIds(entry.missing_capability_ids, `presentation aspect ${id} missing capability IDs`),
        };
    });
}
function assertUniquePresentationId(value, seen, label) {
...
function normalizeRuntimePayloadSchemas(value) {
    if (!Array.isArray(value)) {
    ...
    const knownSchemas = schemas.filter((schema) => BINDING_PAYLOAD_SCHEMAS.some((known) => known.id === schema.id));
    if (knownSchemas.length !== WEB_PAYLOAD_SCHEMA_VERSION_BY_ID.size ||
        knownSchemas.some((schema) => WEB_PAYLOAD_SCHEMA_VERSION_BY_ID.get(schema.id) !== schema.version)) {
        throw new Error("Merman WASM runtime payload schemas do not match the Web transport contract.");
    }
    return schemas;
}
function normalizeRuntimeOutputContracts(value, outputIds) {
    if (!Array.isArray(value)) {
    ...
    const contracts = value.map((entry) => {
        ...
        assertRequiredRecordKeys(entry, ["id", "media_type", "system_fonts", "embedded_images"], "Merman WASM runtime output contract");
    ...
    if (contractIds.length !== outputIds.length ||
...
function normalizeRuntimeSystemFonts(value) {
    if (value === null) {
        return null;
    }
    if (!isRecord(value)) {
    ...
    return {
        ...
        caller_configurable: assertBooleanField(value.caller_configurable, "runtime font configurability"),
        resource_bounded: assertBooleanField(value.resource_bounded, "runtime font resource bound"),
    };
}
function normalizeRuntimeEmbeddedImages(value) {
    if (value === null) {
        return null;
    }
    if (!isRecord(value)) {
    ...
    assertRequiredRecordKeys(value, ["source_ids", "filesystem_access", "network_access", "caller_configurable", "limits"], "Merman WASM runtime embedded-image contract");
    ...
    assertRequiredRecordKeys(limitValues, [...limitNames], "Merman WASM runtime embedded-image limits");
    ...
    return {
        ...
        filesystem_access: assertBooleanField(value.filesystem_access, "runtime embedded-image filesystem access"),
        network_access: assertBooleanField(value.network_access, "runtime embedded-image network access"),
        caller_configurable: assertBooleanField(value.caller_configurable, "runtime embedded-image configurability"),
        limits: {
...
function normalizeRuntimeCapabilities(value) {
    ...
    const operationIds = normalizeSortedIdentifierIds(value.operation_ids, "runtime binding operation IDs");
    const outputIds = normalizeSortedIdentifierIds(value.output_ids, "runtime output IDs");
    const systemAdapterIds = normalizeSortedIdentifierIds(value.system_adapter_ids, "system adapter IDs");
    for (const adapterId of systemAdapterIds) {
        if (!capabilitySet.has(adapterId)) {
            throw new Error(`Merman WASM system adapter ${adapterId} is absent from runtime capability IDs.`);
        }
    }
    ...
    if (svgAvailable && textMeasurement === null) {
        throw new Error("Merman WASM text measurement must be present when SVG is available.");
    }
    ...
    for (const capabilityId of capabilityIds) {
        ...
        for (const implicationId of spec.implication_ids) {
            if (!capabilityIdSet.has(implicationId)) {
                throw new Error(`Merman WASM runtime capability ${capabilityId} is missing implied capability ${implicationId}.`);
            }
        }
    }
}
function validateKnownRuntimeOperationRelations(operationIds, capabilityIds, outputIds) {
    for (const operationId of operationIds) {
        ...
        if (operation.capability !== null && !capabilityIds.has(operation.capability)) {
            throw new Error(`Merman WASM runtime operation ${operationId} is missing capability ${operation.capability}.`);
        }
        if (operation.output !== null && !outputIds.has(operation.output)) {
            throw new Error(`Merman WASM runtime operation ${operationId} is missing output ${operation.output}.`);
        }
    }
}
function normalizeRuntimeResourceContract(value, operationIds) {
    assertRequiredRecordKeys(value, [
    ...
    if (typeof value.general_binding_default_profile !== "string" ||
    ...
    const limits = value.limits.map((limit) => {
        ...
        assertRequiredRecordKeys(limit, [
            "id",
            "phase",
            "description",
            "overridable",
        ...
        const limitOperationIds = normalizeSortedIdentifierIds(limit.operation_ids, `runtime resource limit ${limit.id} operation IDs`);
    ...
    const profiles = value.profiles.map((profile) => {
        ...
        assertRequiredRecordKeys(profile, ["id", "purpose", "trust_assumption", "recommended_binding_default", "limits"], "Merman WASM runtime resource profile");
        ...
        const rawProfileLimits = new Map(normalizeStringMapEntries(profile.limits, `resource profile ${profile.id} limits`));
        if (rawProfileLimits.size !== limits.length ||
        ...
        for (const descriptor of limits) {
            ...
            profileLimits[descriptor.id] =
                limit === null || limit === undefined
                    ? null
                    : assertSafeIntegerField(limit, `resource profile limit ${descriptor.id}`, descriptor.minimum_value);
    ...
    const recommendedProfiles = profiles.filter((profile) => profile.recommended_binding_default);
    if (recommendedProfiles.length !== 1 ||
        recommendedProfiles[0]?.id !== generalDefault.id) {
        throw new Error("Merman WASM runtime resources must recommend exactly the binding default profile.");
    }
    return {
        ...structuredCloneValue(value),
...
function normalizeTextMeasurementCapabilities(value) {
    ...
    assertRequiredRecordKeys(value, ["protocol_version", "provider_ids"], "Merman WASM text measurement capabilities");
    if (typeof value.protocol_version !== "number" ||
    ...
    const providerIds = normalizeSortedIdentifierIds(value.provider_ids, "text measurement provider IDs");
...
function normalizeStringMapEntries(value, label) {
...
function normalizeSortedIdentifierIds(value, label) {
    if (!Array.isArray(value)) {
        throw new Error(`Merman WASM returned invalid ${label}.`);
    }
...
function normalizeSortedOptionGroupIds(value) {
    if (!Array.isArray(value)) {
        throw new Error("Merman WASM returned invalid runtime option group IDs.");
    }
    const identifiers = value.map((entry) => assertRuntimeFieldIdentifier(entry, "runtime option group ID"));
    for (let index = 1; index < identifiers.length; index += 1) {
        if (identifiers[index - 1] >= identifiers[index]) {
...
function normalizeSortedPositiveIntegers(value, label) {
    if (!Array.isArray(value)) {
        throw new Error(`Merman WASM returned invalid ${label}.`);
    }
...
function assertRuntimeIdentifier(value, label) {
    if (typeof value === "string" &&
...
function assertRuntimeFieldIdentifier(value, label) {
    if (typeof value === "string" &&
...
function assertSafeIntegerField(value, label, minimum) {
    if (typeof value === "number" && isSafeCatalogInteger(value) && value >= minimum) {
        return value;
    }
    throw new Error(`Merman WASM returned an invalid ${label}.`);
}
function isSafeCatalogInteger(value) {
    return Number.isInteger(value) && Math.abs(value) <= RUNTIME_CATALOG_MAX_SAFE_INTEGER;
}
function assertRequiredRecordKeys(value, required, label) {
