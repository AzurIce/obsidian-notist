...
import { assertNavigableSvgForDom, prepareNavigableSvgForDomMount, } from "./svg-safety.js";
export function renderSvg(source, options) {
    return getMerman().renderSvg(source, encodeOptions(options));
}
export function svgPlanJson(source, options) {
    return getMerman().svgPlanJson(source, encodeOptions(options));
}
export function renderSvgWithTextMeasurer(source, measurer, options) {
    const renderWithMeasurer = getMerman().renderSvgWithTextMeasurer;
    if (!renderWithMeasurer) {
        throw new Error("Merman WASM does not expose renderSvgWithTextMeasurer(). Rebuild @mermanjs/web.");
    }
    return renderWithMeasurer(source, encodeOptions(options), measurer);
}
export function layoutJsonWithTextMeasurer(source, measurer, options) {
    const layoutWithMeasurer = getMerman().layoutJsonWithTextMeasurer;
    if (!layoutWithMeasurer) {
        throw new Error("Merman WASM does not expose layoutJsonWithTextMeasurer(). Rebuild @mermanjs/web.");
    }
    return layoutWithMeasurer(source, encodeOptions(options), measurer);
}
export function createBrowserTextMeasurementSession() {
    let probes = null;
    let disposed = false;
    const measure = (request) => {
        ...
        try {
            probes ?? (probes = createTextMeasureProbes());
            if (!probes) {
                return undefined;
            }
...
function measureWithBrowserProbes(probes, request) {
    if (!request.text) {
...
function emptyMeasurement(request) {
    switch (request.operation) {
        case "bbox-x":
        case "bbox-x-with-ascii-overhang":
        case "title-bbox-x":
...
function mermaidCalculateTextDimensions(probes, request) {
...
function canvasTextWidth(probes, request) {
...
function wrappedMetrics(probes, request) {
...
function htmlWrappedMetrics(probe, request) {
...
function svgWrappedMetrics(probes, request) {
    const maxWidth = normalizeMeasureMaxWidth(request);
    const breakLongWords = request.wrap_mode === "svg-like";
    const lines = splitExplicitLines(request.text).flatMap((line) => maxWidth === null
        ? [line]
...
function svgCreateTextBBoxYOffset(probes, request, middleBaseline) {
...
function splitTextToGraphemes(text) {
    if (typeof Intl.Segmenter === "function") {
        return [...new Intl.Segmenter().segment(text)].map(({ segment }) => segment);
    }
    return Array.from(text);
}
function wrapSvgLine(probes, request, line, maxWidth, breakLongWords) {
...
function svgBBox(probes, request, shape, anchor = "start") {
    const element = prepareSvgText(probes, request, shape, anchor);
    return element.getBBox();
}
function prepareSvgText(probes, request, shape, anchor) {
...
function applyHtmlTextMeasureStyle(probe, request) {
...
function applySvgTextStyle(element, request) {
...
function measureHtmlProbe(probe, text, styleOverride) {
...
function setHtmlProbeText(probe, text) {
...
function normalizeMeasureMaxWidth(request) {
    if (!request.has_max_width ||
...
function createTextMeasureProbes() {
    ...
    try {
        ...
        const directText = document.createElementNS(SVG_NAMESPACE, "text");
        const tspanText = document.createElementNS(SVG_NAMESPACE, "text");
        const tspan = document.createElementNS(SVG_NAMESPACE, "tspan");
        const wrappedText = document.createElementNS(SVG_NAMESPACE, "text");
        const formattedTextGroup = document.createElementNS(SVG_NAMESPACE, "g");
        const formattedText = document.createElementNS(SVG_NAMESPACE, "text");
        ...
        mermaidDimensionsSvg = document.createElementNS(SVG_NAMESPACE, "svg");
        mermaidDimensionsSvg.setAttribute("aria-hidden", "true");
        mermaidDimensionsSvg.setAttribute("data-merman-text-measure-probe", "mermaid-dimensions");
        ...
        const mermaidDimensionsText = document.createElementNS(SVG_NAMESPACE, "text");
        const mermaidDimensionsTspan = document.createElementNS(SVG_NAMESPACE, "tspan");
...
function disposeTextMeasureProbes(probes) {
...
function removeTextMeasureProbe(probe) {
    try {
        probe?.remove();
    }
    catch {
...
function renderSvgElementInDocument(ownerDocument, source, options) {
    ...
    const imported = ownerDocument.importNode(svg, true);
    prepareNavigableSvgForDomMount(admission, imported, ownerDocument);
    return imported;
}
export function renderSvgToElement(target, source, options) {
...
export function parseObject(source, options) {
    return JSON.parse(parseJson(source, options));
}
export function layoutJson(source, options) {
    return getMerman().layoutJson(source, encodeOptions(options));
}
export function layoutObject(source, options) {
    return JSON.parse(layoutJson(source, options));
}
//# sourceMappingURL=runtime-render.js.map