...
const SELF_CONTAINED_SVG_POLICY = Object.freeze({
    anchorNavigation: "fragment-only",
});
const NAVIGABLE_SVG_POLICY = Object.freeze({
    anchorNavigation: "mermaid-compatible",
});
export function assertSelfContainedSvgWithMessagePrefix(svg, messagePrefix) {
    const scanner = new SvgSafetyScanner(svg, messagePrefix, SELF_CONTAINED_SVG_POLICY);
    return scanner.assertSafe();
}
export function assertNavigableSvgWithMessagePrefix(svg, messagePrefix) {
    const scanner = new SvgSafetyScanner(svg, messagePrefix, NAVIGABLE_SVG_POLICY);
    return scanner.assertSafe();
}
class SvgSafetyScanner {
    constructor(source, messagePrefix, policy) {
        this.source = source;
        ...
        this.sawRoot = false;
        this.rootDepth = 0;
        this.rootClosedAt = null;
        this.foreignObjectDepth = 0;
        this.embeddedImageEncodedBytes = 0;
        this.embeddedImageBytes = 0;
        this.embeddedImagePixels = 0;
        this.hasFragmentReferences = false;
        this.elementStack = [];
    }
    assertSafe() {
        if (this.source.length > MAX_SAFE_SVG_SOURCE_CODE_UNITS ||
        ...
        while (this.cursor < this.source.length) {
            ...
            this.assertSafeAttributes(elementName, tag.attributes, inForeignObject || elementName === "foreignobject");
    ...
    assertSafeElementName(name, inForeignObject) {
    ...
    assertSafeAttributes(elementName, attributes, inForeignObject) {
    ...
    assertSafeAnchorInteractionAttribute(name, value) {
        if (name === "download") {
    ...
    assertSafeUrl(value, source, elementName, attributeName) {
    ...
    assertSafeAnchorNavigationUrl(rawValue, decodedValue, normalized, compact, trimmed) {
    ...
    assertSafeEmbeddedRasterDataUrl(value) {
        ...
        if (inspected.width > Math.floor(MAX_EMBEDDED_IMAGE_PIXELS / inspected.height)) {
    ...
    assertSafeCss(css) {
        ...
        if (containsCssFunction(lower, "image-set") ||
    ...
    assertSafeUrlReferences(value, attributeName) {
    ...
    assertOnlyIgnorableRootTail(start) {
        let cursor = start;
        while (cursor < this.source.length) {
            cursor = skipWhitespace(this.source, cursor);
...
function parseRasterDataUrl(value) {
    ...
    return {
        declaredFormat: mime === "jpg" ? "jpeg" : mime,
        payload,
        encodedBytes: payload.length,
        decodedBytes,
    };
}
function decodeBase64(payload, decodedBytes) {
...
function validPngHeader(header) {
        ...
        : colorType === 2
            ...
            : colorType === 3
                ...
                : colorType === 4 || colorType === 6
                    ? [8, 16]
                    : [];
    return (bitDepth !== undefined &&
        allowedDepths.includes(bitDepth) &&
        header[10] === 0 &&
        header[11] === 0 &&
        (header[12] === 0 || header[12] === 1));
}
function inspectPngInternationalTextCompression(bytes, start, length) {
...
function findByte(bytes, start, end, expected) {
...
function asciiSlice(bytes, offset, length) {
...
function containsCssFunction(css, name) {
    let cursor = 0;
    while (cursor < css.length) {
...
function containsShadowScopingSelector(css) {
    return css.includes(":host") || css.includes("::slotted");
}
function containsViewportEscapingCssDeclaration(css) {
    return cssDeclarationBlocks(css).some((block) => {
        if (/(?:^|[;{\s])position\s*:\s*(?:fixed|sticky)\b/.test(block)) {
            return true;
        }
        return (/(?:^|[;{\s])position\s*:\s*absolute\b/.test(block) &&
            /(?:^|[;{\s])(?:inset(?:-(?:block|inline)(?:-(?:start|end))?)?|top|right|bottom|left)\s*:/.test(block));
    });
}
...
function decodeNumericCharacterReference(value, ampersand) {
...
function decodeNamedCharacterReference(value, ampersand) {
    ...
    const replacement = NAMED_CHARACTER_REFERENCES.get(value.slice(ampersand + 1, cursor).toLowerCase());
    if (replacement === undefined) {
        return null;
    }
    return { value: replacement, end: cursor + 1 };
}
function containsCharacterReference(value) {
...
function removeAsciiWhitespaceAndControl(value) {
    let output = "";
...
function findClosingStyle(source, start) {
...
function matchesAsciiCaseInsensitive(source, start, lowerAscii) {
...
function isCssIdentifierChar(char) {
...
function isNameTerminator(char) {
    return isWhitespace(char) || char === ">" || char === "/";
}
function isAttributeNameTerminator(char) {
    return isWhitespace(char) || char === "=" || char === ">" || char === "/";
}
function isUnquotedValueTerminator(char) {
    return isWhitespace(char) || char === ">";
}
//# sourceMappingURL=svg-safety-policy.js.map